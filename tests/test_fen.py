...
class FENTests(unittest.TestCase):
    ...
    def test_fen_after_1_e4_c5(self):
        ...
        m = pychess.util.move.from_algebraic(board, PieceColor.WHITE, 'e4')
        board.move(m)
        m = pychess.util.move.from_algebraic(board, PieceColor.BLACK, 'c5')
        board.move(m)
    ...
    def test_fen_after_1_e4_c5_2_Nf3(self):
        ...
        m = pychess.util.move.from_algebraic(board, PieceColor.BLACK, 'c5')
        board.move(m)
        m = pychess.util.move.from_algebraic(board, PieceColor.WHITE, 'Nf3')
        board.move(m)
